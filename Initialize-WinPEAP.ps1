...
.DESCRIPTION
    ...
    This script only adds the WinPEAP-specific pieces, using the documented customization surface:
        ...
        OSDRepo\winpeap-media.ps1           media script - stages the startup files DURING the build
        OSDRepo\build-profiles\amd64\<name>.json   seeded from the stock profile;
                                                  WinPEStartupProfile + WinPEMediaScript overridden
        OSDRepo\Invoke-WinPEAPBuild.ps1     build wrapper
...
# ---------- 6b. media script (stages the startup files DURING the build, before the ISO) ----------
$mediaScript = Join-Path $OSDeployRoot 'winpeap-media.ps1'
@"
# Generated by Initialize-WinPEAP.ps1 - runs during Build-OSDeployBoot (media phase).
# OSDeploy exposes `$global:BuildMedia.MediaPath / .MediaPathEX
`$ErrorActionPreference = 'Continue'

# 1) run the stock EN-US language filter from the installed OSDeploy module (size trim)
`$mod = (Get-Module OSDeploy).ModuleBase
if (-not `$mod) { `$mod = (Get-Module OSDeploy -ListAvailable | Select-Object -First 1).ModuleBase }
`$enus = Join-Path `$mod 'core\OSDRepo\media-script\language-filter\BootMedia - EN-US filter.ps1'
if (Test-Path `$enus) { & `$enus } else { Write-Warning '[WinPEAP] EN-US language filter not found; skipping' }

# 2) stage the WinPEAP startup files onto the boot media
`$src = '$filesDir'
foreach (`$mp in @(`$global:BuildMedia.MediaPath, `$global:BuildMedia.MediaPathEX)) {
    if ([string]::IsNullOrWhiteSpace(`$mp)) { continue }
    `$dst = Join-Path `$mp 'WinPEStartup\Files'
    New-Item `$dst -ItemType Directory -Force | Out-Null
    robocopy `$src `$dst /E /NFL /NDL /NJH /NJS | Out-Null
    Write-Host "[WinPEAP] staged startup files -> `$dst"
}
"@ | Set-Content $mediaScript -Encoding UTF8
Say "winpeap-media.ps1 (runs EN-US filter + stages startup files during Build-OSDeployBoot)" Green
...
$o | Add-Member NoteProperty WinPEMediaScript    $mediaScript -Force   # replaces the stock EN-US filter (our script calls it)
