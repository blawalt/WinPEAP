> **Legacy guide — OSDCloud V1** (the `OSD` module: `Edit-OSDCloudWinPE`, `-StartOSDCloud`, `-ZTI`).
> Still supported. For the workflow-based V2 engine and the hydration kit, see **[OSDCloud-V2.md](OSDCloud-V2.md)**.
...
2.  **Copy Required Files:** Place the following files into this `Startnet` directory:
    * `4kAutopilotHashUpload.ps1`: The custom PowerShell script responsible for capturing and uploading the hash (you MUST update all variables at beginning of script).
